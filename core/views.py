...
class ControladorInscricao(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.POST.get("passo") == "selecionar":
            ...
            try:
                ...
                ofertas_cursando_ids = aluno.participacao_set.filter(
                    status="C"
                ).values_list("ofertaDisciplina", flat=True)
                ofertas_cursando = OfertaDisciplina.objects.filter(
                    id__in=ofertas_cursando_ids
                )
                return self.realizar_inscricao(
                    request,
                    context,
                    aluno,
                    ofertas,
    ...
    def realizar_inscricao(
        ...
        if not ofertas:
            return redirect("inscricao", aluno_id=aluno.id)
        ...
        # a inscrição não será realizada e a mensagem de erro será exibida
        creditos = self._get_creditos(aluno, ofertas)
        if creditos > 20:
        ...
        for oferta in ofertas:
            cursando = oferta.aluno_esta_inscrito(aluno)
            if not cursando:
                oferta.inscrever_aluno(aluno)
    ...
    def _get_creditos(self, aluno, ofertas):
        creditos_ofertas = sum([oferta.creditos() for oferta in ofertas])
        creditos_cursando = aluno.get_creditos()
        return creditos_ofertas + creditos_cursando
